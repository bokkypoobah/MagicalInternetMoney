      ...
      <b-modal ref="viewaddress" v-model="show" hide-footer header-class="m-0 px-3 py-2" body-bg-variant="light" size="lg">
        ...
        <b-form-group label="Address:" label-for="address-address" label-size="sm" label-cols-sm="3" label-align-sm="right" class="mx-0 my-1 p-0">
          <b-input-group size="sm" class="w-100">
            ...
            <b-input-group-append>
              <div>
                <b-button size="sm" :href="explorer + 'address/' + address" variant="link" v-b-popover.hover.ds500="'View in explorer'" target="_blank" class="m-0 ml-1 p-0"><b-icon-link45deg shift-v="+1" font-scale="0.95"></b-icon-link45deg></b-button>
              </div>
            </b-input-group-append>
          </b-input-group>
        </b-form-group>
        ...
        <b-form-group v-if="type == 'stealthAddress'" label="Linked To Address:" label-for="address-linkedtoaddress" label-size="sm" label-cols-sm="3" label-align-sm="right" class="mx-0 my-1 p-0">
          <b-input-group size="sm" class="w-100">
            ...
            <b-input-group-append>
              <div>
                <b-button size="sm" :href="explorer + 'address/' + linkedTo.address" variant="link" v-b-popover.hover.ds500="'View in explorer'" target="_blank" class="m-0 ml-1 p-0"><b-icon-link45deg shift-v="+1" font-scale="0.95"></b-icon-link45deg></b-button>
              </div>
            </b-input-group-append>
          </b-input-group>
        </b-form-group>
        ...
        <b-form-group v-if="type == 'stealthAddress'" label="Created Via Stealth Transfers:" label-for="address-stealthtransfers" label-size="sm" label-cols-sm="3" label-align-sm="right" class="mx-0 my-1 p-0">
          ...
          <b-row v-for="(item, index) of stealthTransfers" v-bind:key="item.txHash" class="px-2">
            <b-col cols="4" class="px-0 mt-1">
              <b-button size="sm" :href="explorer + 'tx/' + item.txHash" variant="link" v-b-popover.hover.ds500="'View in explorer'" target="_blank" class="m-0 ml-1 p-0 px-2">
                {{ formatTimestamp(item.timestamp) }}
              </b-button>
            </b-col>
            <b-col cols="4" class="px-0 mt-1">
              <b-button v-if="item.tx && item.tx.from" size="sm" :href="explorer + 'address/' + item.tx.from" variant="link" v-b-popover.hover.ds500="'View in explorer'" target="_blank" class="m-0 ml-1 p-0 px-2">
        ...
        <b-form-group label="" label-size="sm" label-cols-sm="3" label-align-sm="right" class="mx-0 mt-1 mb-2 p-0">
          ...
          <b-button size="sm" :disabled="junk" id="address-mine" :pressed.sync="mine" variant="transparent" v-b-popover.hover.ds500="mine ? 'My account' : 'Not my account'" class="m-0 mx-2 p-0">
            <b-icon :icon="(mine && !junk) ? 'person-fill' : 'person'" shift-v="+1" font-scale="1.2" :variant="(junk || !mine) ? 'secondary' : 'primary'">
            </b-icon>
          </b-button>
          <b-button size="sm" :disabled="junk" id="address-watch" :pressed.sync="watch" variant="transparent" v-b-popover.hover.ds500="(watch ? 'Watch' : 'Do not watch') + ' this address for ETH, ERC-20, ERC-721 and ERC-1155 transfers'" class="m-0 mx-2 p-0">
            <b-icon :icon="(watch && !junk) ? 'eye-fill' : 'eye'" shift-v="+1" font-scale="1.2" :variant="(junk || !watch) ? 'secondary' : 'primary'">
            </b-icon>
          </b-button>
          <b-button size="sm" :disabled="junk || !mine" id="address-sendfrom" :pressed.sync="sendFrom" variant="transparent" v-b-popover.hover.ds500="'ETH and tokens ' + (sendFrom ? 'can' : 'cannot') + ' be sent from this address'" class="m-0 mx-2 p-0">
          ...
          <b-button size="sm" :disabled="junk" id="address-sendto" :pressed.sync="sendTo" variant="transparent" v-b-popover.hover.ds500="'ETH and tokens ' + (sendTo ? 'can' : 'cannot') + ' be sent to this address'" class="m-0 mx-2 p-0">
        ...
        <b-form-group v-if="ensName" label="ENS Name:" label-for="address-ensname" label-size="sm" label-cols-sm="3" label-align-sm="right" class="mx-0 my-1 p-0">
          <b-input-group size="sm" class="w-100">
            ...
            <b-input-group-append>
              <div>
                <b-button size="sm" :href="'https://app.ens.domains/' + ensName" variant="link" v-b-popover.hover.ds500="'View in ENS dapp'" target="_blank" class="m-0 ml-1 p-0"><b-icon-link45deg shift-v="+1" font-scale="0.95"></b-icon-link45deg></b-button>
              </div>
            </b-input-group-append>
          </b-input-group>
        </b-form-group>
        ...
        <b-form-group label="Name:" label-for="address-name" label-size="sm" label-cols-sm="3" label-align-sm="right" class="mx-0 my-1 p-0">
          <b-input-group size="sm" class="w-75">
            ...
            <b-input-group-append>
              <div>
                <b-button size="sm" :pressed.sync="mine" variant="transparent" v-b-popover.hover.ds500="addressTypeInfo[type || 'address'].name" class="m-0 ml-5 p-0"><b-icon :icon="mine ? 'star-fill' : 'star'" shift-v="+1" font-scale="0.95" :variant="addressTypeInfo[type || 'address'].variant"></b-icon></b-button>
                <b-button size="sm" :pressed.sync="favourite" variant="transparent" v-b-popover.hover.ds500="'Favourite?'" class="m-0 ml-1 p-0"><b-icon :icon="favourite ? 'heart-fill' : 'heart'" shift-v="+1" font-scale="0.95" variant="danger"></b-icon></b-button>
              </div>
            </b-input-group-append>
            -->
          </b-input-group>
        ...
        <b-form-group v-if="address" label="" label-for="address-delete" label-size="sm" label-cols-sm="3" label-align-sm="right" class="mx-0 my-1 p-0">
          <b-button size="sm" @click="deleteAddress(address);" variant="link" v-b-popover.hover.ds500="'Delete address ' + address.substring(0, 10) + '...' + address.slice(-8) + '?'"><b-icon-trash shift-v="+1" font-scale="1.1" variant="danger"></b-icon-trash></b-button>
        </b-form-group>
      </b-modal>
    </div>
  `,
