    ...
    <div class="m-0 p-0">
      <b-card no-body no-header class="border-0">
        <div class="d-flex flex-wrap m-0 p-0">
          ...
          <div class="mt-0 pr-1">
            <font size="-2" v-b-popover.hover.ds500.dh50="'# registry entries'">{{ filteredSortedRegistryEntries.length + '/' + totalRegistryEntries }}</font>
          </div>
          <div class="mt-0 pr-1">
        ...
        <b-table ref="registryTable" small fixed striped responsive hover :fields="fields" :items="pagedFilteredSortedRegistryEntries" show-empty head-variant="light" class="m-0 mt-1">
          ...
          <template #cell(registrant)="data">
            <b-link :href="explorer + 'address/' + data.item.registrant" v-b-popover.hover.ds500.dh50="data.item.registrant" target="_blank">
              {{ names[data.item.registrant] || (data.item.registrant.substring(0, 8) + '...' + data.item.registrant.slice(-6)) }}
            </b-link>
          </template>
          <template #cell(transfer)="data">
            <b-button size="sm" :disabled="!transferHelper" @click="newTransfer(data.item.stealthMetaAddress);" variant="link" v-b-popover.hover.ds500.dh50="'Transfer to ' + data.item.stealthMetaAddress" class="m-0 ml-2 p-0"><b-icon-caret-right shift-v="+1" font-scale="1.1"></b-icon-caret-right></b-button>
          </template>
