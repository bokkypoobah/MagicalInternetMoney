  ...
  template: `
    <div>
      ...
      <b-modal ref="addtokensfortransfer" id="modal-addtokensfortransfer" hide-footer body-bg-variant="light" size="md">
        ...
        <b-form-select size="sm" v-model="modalAddTokensToNewTransfer.token" @change="additionalTokenSelected" :options="additionalERC20TokensOptions" v-b-popover.hover.ds500="'Select ERC-20 or ERC-721 tokens to transfer'" class="w-100"></b-form-select>
...
{{ modalAddTokensToNewTransfer }}
      ...
      <b-modal ref="newtransfer" v-model="show" id="modal-newtransfer" hide-footer header-class="m-0 px-3 py-2" body-bg-variant="light" size="lg">
        ...
        <b-form-group v-if="!stealthMetaAddressSpecified" label="To:" label-for="newtransfer-to-unspecified" label-size="sm" label-cols-sm="3" label-align-sm="right" class="mx-0 my-1 p-0">
          <b-form-select size="sm" v-model="stealthMetaAddress" :options="stealthMetaAddressesOptions" v-b-popover.hover.ds500="'Select from your approved Stealth Meta-Addresses'" class="w-100"></b-form-select>
        </b-form-group>
          ...
          <b-row v-for="(item, index) of items" v-bind:key="index">
            ...
            <b-col cols="3" class="mb-1 pr-1">
              ...
              <div v-if="item.type == 'erc721'">
                <b-button size="sm" :href="'https://testnets.opensea.io/assets/sepolia/' + item.token + '/' + item.tokenId" variant="link" v-b-popover.hover.ds500="item.tokenId" class="m-0 ml-2 p-0" target="_blank">{{ item.tokenId.toString().length > 20 ? (item.tokenId.toString().substring(0, 8) + '...' + item.tokenId.toString().slice(-8)) : item.tokenId.toString() }}</b-button>
              </div>
            </b-col>
            <b-col cols="1" class="mx-0 px-1">
              <b-button size="sm" @click="removeTokensForTransfer(index);" variant="link" v-b-popover.hover.ds500="'Remove ' + item.symbol + ' ' + (item.type == 'erc20' ? item.amount : item.tokenId)" class="m-0 ml-2 p-0"><b-icon-dash shift-v="+1" font-scale="1.1"></b-icon-dash></b-button>
            </b-col>
          </b-row>
          <b-row>
            <b-col cols="3" class="mx-0 px-1 text-right">
            ...
            <b-col cols="1" class="mx-0 px-1">
              <b-button size="sm" @click="addTokensForTransfer();" variant="link" v-b-popover.hover.ds500="'Add ERC-20/ERC-721 tokens to transfer'" class="m-0 ml-2 p-0"><b-icon-plus shift-v="+1" font-scale="1.1"></b-icon-plus></b-button>
            </b-col>
          </b-row>
        </b-form-group>
  ...
  computed: {
    ...
    filteredERC721Tokens() {
      console.log(now() + " INFO NewTransfer:methods.filteredERC721Tokens BEGIN");
      const results = [];
      ...
      console.log(now() + " INFO NewTransfer:methods.filteredERC721Tokens - tokenContract: " + JSON.stringify(tokenContract, null, 2));
      ...
      console.log(now() + " INFO NewTransfer:methods.filteredERC721Tokens - results: " + JSON.stringify(results, null, 2));
      return results;
    },
    ...
    pagedFilteredSortedERC721Tokens() {
      console.log(now() + " INFO NewTransfer:methods.pagedFilteredSortedERC721Tokens - results[0..9]: " + JSON.stringify(this.filteredSortedERC721Tokens.slice(0, 10), null, 2));
  ...
  methods: {
    saveSettings() {
      console.log(now() + " INFO NewTransfer:methods.saveSettings - newTransferSettings: " + JSON.stringify(this.settings, null, 2));
    ...
    addTokensForTransfer() {
      console.log(now() + " INFO NewTransfer:methods.addTokensForTransfer");
      this.$bvModal.show('modal-addtokensfortransfer');
    },
    async additionalTokenSelected() {
      console.log(now() + " INFO NewTransfer:methods.additionalTokenSelected");
      console.log(now() + " INFO NewTransfer:methods.additionalTokenSelected - this.modalAddTokensToNewTransfer: " + JSON.stringify(this.modalAddTokensToNewTransfer, null, 2));
    ...
    async updateApproval() {
      console.log(now() + " INFO NewTransfer:methods.updateApproval BEGIN: " + JSON.stringify(this.modalAddTokensToNewTransfer, null, 2));
      const provider = new ethers.providers.Web3Provider(window.ethereum);
      ...
      console.log(now() + " INFO NewTransfer:methods.updateApproval END");
    },

    erc721TokenIdToggle(tokenId) {
      console.log(now() + " INFO NewTransfer:methods.erc721TokenIdToggle - tokenId: " + tokenId);
      ...
      console.log(now() + " INFO NewTransfer:methods.erc721TokenIdToggle - this.modalAddTokensToNewTransfer.selectedERC721TokenIds: " + JSON.stringify(this.modalAddTokensToNewTransfer.selectedERC721TokenIds));
    },

    addERC20TokenToTransfer() {
      console.log(now() + " INFO NewTransfer:methods.addERC20TokenToTransfer");
      const tokenContract = this.tokens[this.chainId] && this.tokens[this.chainId][this.modalAddTokensToNewTransfer.token] || {};
      const decimals = tokenContract.decimals && parseInt(tokenContract.decimals) || null;
    ...
    addERC721TokenToTransfer() {
      console.log(now() + " INFO NewTransfer:methods.addERC721TokenToTransfer");
      const tokenContract = this.tokens[this.chainId] && this.tokens[this.chainId][this.modalAddTokensToNewTransfer.token] || {};
      const balances = this.balances[this.chainId] && this.balances[this.chainId][this.modalAddTokensToNewTransfer.token] || {};
      // console.log("tokenContract: " + JSON.stringify(tokenContract, null, 2));
    ...
    removeTokensForTransfer(index) {
      console.log(now() + " removeTokensForTransfer: " + index);
    ...
    async executeNewTransfer() {
      ...
      console.log(now() + " INFO NewTransfer:methods.executeNewTransfer BEGIN");
      ...
      console.log(now() + " INFO NewTransfer:methods.executeNewTransfer END");
    },
    async syncIt(info) {
...
const newTransferModule = {
  ...
  mutations: {
    newTransfer(state, stealthMetaAddress) {
      console.log(now() + " INFO newTransferModule:mutations.newTransfer - stealthMetaAddress: " + stealthMetaAddress);
      state.show = true;
    ...
    setStealthMetaAddress(state, stealthMetaAddress) {
      console.log(now() + " INFO newTransferModule:mutations.setStealthMetaAddress - stealthMetaAddress: " + stealthMetaAddress);
      state.stealthMetaAddress = stealthMetaAddress;
    },
    setAmount(state, amount) {
      console.log(now() + " INFO newTransferModule:mutations.setAmount - amount: " + amount);
      state.amount = amount;
    },
    addItem(state, item) {
      console.log(now() + " INFO newTransferModule:mutations.addItem - item: " + JSON.stringify(item));
      state.items.push(item);
    },
    removeItem(state, index) {
      console.log(now() + " INFO newTransferModule:mutations.removeItem - index: " + JSON.stringify(index));
      state.items.splice(index, 1);
    },
  },
  actions: {
    async newTransfer(context, stealthMetaAddress) {
      console.log(now() + " INFO newTransferModule:actions.newTransfer - stealthMetaAddress: " + stealthMetaAddress);
    ...
    async setStealthMetaAddress(context, stealthMetaAddress) {
      console.log(now() + " INFO newTransferModule:actions.setStealthMetaAddress - stealthMetaAddress: " + stealthMetaAddress);
      await context.commit('setStealthMetaAddress', stealthMetaAddress);
    },
    async setAmount(context, amount) {
      console.log(now() + " INFO newTransferModule:actions.setAmount - amount: " + amount);
      await context.commit('setAmount', amount);
    },
    async addItem(context, item) {
      console.log(now() + " INFO newTransferModule:actions.addItem - item: " + JSON.stringify(item));
      await context.commit('addItem', item);
    },
    async removeItem(context, index) {
      console.log(now() + " INFO newTransferModule:actions.removeItem - index: " + JSON.stringify(index));
