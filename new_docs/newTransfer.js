...
{{ transfer }}
  ...
  methods: {
    ...
    rowSelected(item) {
      logInfo("NewTransfer", "methods.rowSelected BEGIN: " + JSON.stringify(item, null, 2));
...
const newTransferModule = {
  ...
  mutations: {
    ...
    deQueue(state) {
      logDebug("newTransferModule", "deQueue(" + JSON.stringify(state.executionQueue) + ")");
      state.executionQueue.shift();
    },
    updateParams(state, params) {
      state.params = params;
      logDebug("newTransferModule", "updateParams('" + params + "')")
    },
    updateExecuting(state, executing) {
      state.executing = executing;
      logDebug("newTransferModule", "updateExecuting(" + executing + ")")
    },
  },
  actions: {
